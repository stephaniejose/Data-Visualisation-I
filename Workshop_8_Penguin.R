...
#The dataframe has a number of different measurements for three penguin species across three different islands 
View(penguins)
...
#They changed the intervals in the y-axis and this creates more overlap in the RHS graph
...
#in the above code we used a mix of pipe ( %>% ) and '+', these can get commonly mixed up 
...
#[7] LABELS
#This makes the plot look better by adding labels using the function labs()
...
#This can't be done with labs() because the legend is part of scales
#In this case we have mapped a categorical plot so we can use scale_fill_discrete(), this allows us to change the colours
